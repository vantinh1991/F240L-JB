...
#else
				//LDBGI("%s: TEST BURSTSHOT Enabled!\n", __func__);
				wdata[0] = 0x20;
              wdata[1] = 0x00;
              ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x07, wdata,  2);
...
#if 0
       if(scene_mode == CAMERA_SCENE_NIGHT){
         //EV parameters
          data[0] = 0x01;
          data[1] = 0x0D;
         ...
         //sharpness parameters
          data[0] = 0x02;
          data[1] = 0x03;
          ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x3D, data,  2);
          ...
          do {
            mdelay(5); //yt.jeon 1115 optimize delay time
            //rdata[0] = 0x00;
            rc = ce1702_i2c_read(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x02, NULL, 0, &result, 1);
            if (rc < 0)
              LDBGE("%s: ce1702_i2c_read failed \n", __func__);
            LDBGI("%s: verifying... cnt=%d rc=%d\n", __func__, cnt, rc);
            cnt++;
          } while ( (result != 0) && (cnt < 100) );           
      }else if(ce1702_scene_mode == CAMERA_SCENE_NIGHT){      
         //EV parameters
          data[0] = 0x01;
          data[1] = 0x00;
          ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x04, data,  2);
