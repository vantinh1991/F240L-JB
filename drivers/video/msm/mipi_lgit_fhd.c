...
#if defined(CONFIG_MACH_APQ8064_GKU) || defined(CONFIG_MACH_APQ8064_GKATT)
...
#elif defined(CONFIG_MACH_APQ8064_GKKT) || defined(CONFIG_MACH_APQ8064_GKSK)
...
int mipi_lgit_lcd_on(struct platform_device *pdev)
{
	...
	if(first_boot_flag==false)
	{
		...
		if((get_inversion_mode()==INVERSION_MODE_DEFAULT) && (get_current_inversion()!=INVERSION_NORMAL)){
				...
				printk("[LCD][DEBUG] %s rtc_interval:%lu  rtc_time_current:%lu  rtc_time_initial:%lu \n",__func__, rtc_time_interval, rtc_time_current, rtc_time_initial);
				...
				if( rtc_time_interval<=0  || rtc_time_interval >=CHANGE_INVERSION_INTERVAL ){
					...
					schedule_delayed_work(&lcd_worker,rtc_time_interval*HZ);
